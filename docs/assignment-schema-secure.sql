...
-- Secure version compatible with Clerk authentication and existing schema
-- Uses owner_id and collaborators table for proper access control
...
-- These policies check if user is either the playbook owner OR an accepted collaborator
ALTER TABLE assignments ENABLE ROW LEVEL SECURITY;

-- Policy: Users can view assignments for playbooks they have access to
-- (either as owner or as accepted collaborator)
...
-- (owner or collaborators with edit/admin permission)
...
-- Policy: Users can update assignments they created or are assigned to, or if they own/can edit the playbook
...
CREATE POLICY "Users can update relevant assignments" ON assignments
  FOR UPDATE USING (
    ...
    OR
    -- User is a collaborator with edit/admin permission
    EXISTS (
      SELECT 1 FROM playbooks p
...
-- Policy: Users can delete assignments they created or if they own the playbook
...
-- This will be handled by your backend service with service role key
...
BEGIN
  ...
  RAISE NOTICE '✓ RLS policies: Secure policies using owner_id and collaborators table';
  RAISE NOTICE '✓ Access control: Owner and accepted collaborators can view/manage assignments';
  RAISE NOTICE '✓ Next step: Run assignment-schema-update.sql for multiple assignees support';
END $$;
