...
interface PlaybookEditorProps {
      ...
      generatedContent?: any  // Content generated by AI for the current page
}
...
export default function PlaybookEditor({
  ...
  // Notify parent component when current page changes
  useEffect(() => {
    if (onCurrentPageChange) {
      onCurrentPageChange(currentPageId)
    }
  }, [currentPageId, onCurrentPageChange])

  // Handle generated content for internal pages
  useEffect(() => {
    if (generatedContent && currentPageId) {
      console.log('Received generated content for internal page:', currentPageId, generatedContent)
      // Update the internal page content
      const internalPage = internalPages.find(page => page.id === currentPageId)
      if (internalPage) {
        // Update the internal page content
        const updatedPage = { ...internalPage, content: generatedContent }
        setInternalPages(prev => prev.map(page => 
          page.id === currentPageId ? updatedPage : page
        ))
        
        // Save to database
        internalPageService.updateInternalPage(currentPageId, {
          content: generatedContent
        }).catch(error => {
          console.error('Error saving generated content to internal page:', error)
        })
      }
    }
